...
class WarehouseLocationItems
{
  ...
  {
    $this->model->with('itemGroup', 'itemCategory', 'unit','batchs');
    $this->byBranch();
    $this->searchColumns();
  ...
  {
    $warehouse = Request()->warehouse_idd ? Request()->warehouse_idd : Auth::user()->warehouse_id;
    if ($warehouse) {
      $this->model->with('wareHouseItems')->whereHas('wareHouseItems', function ($query) use ($warehouse) {
        $query->where('item_OnHand','>',0)->where('warehouse_Id', $warehouse)->where('branch_id', Auth::user()->branch_id);
      });
    }
  }
