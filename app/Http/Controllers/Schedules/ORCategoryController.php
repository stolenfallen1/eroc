...
use App\Models\Schedules\ORCategory;
...
class ORCategoryController extends Controller
{
    ...
    {
        $casetype = ORCategory::where('category_name', $request->payload['category_name'])->first();
        if (!$casetype) {
            // The site doesn't exist, so create a new one
            $casetype = ORCategory::create([
