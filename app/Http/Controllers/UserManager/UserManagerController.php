...
class UserManagerController extends \TCG\Voyager\Http\Controllers\Controller
{
    ...
    public function update(Request $request, $id)
    {
        try{
            ...
            if(!empty($request->subsystem)){
                Systemuseraccess::where('user_id',$request->id)->delete();
            
                $form = [];
                foreach($request->subsystem as $row){
                $form[] = array(
                    'subsystem_id' => $row,
                    'user_id' => $request->id,
            ...
            return redirect()->back()->with([
                'message'    =>"Usercode is already used {$dataType->getTranslatedAttribute('display_name_singular')}",
                'alert-type' => 'error',
            ]);
        }   
    }
    ...
    public function store(Request $request)
    {
        try {
            ...
            if (!empty($request->subsystem)) {
                Systemuseraccess::where('user_id', $data['id'])->delete();
                $form = [];
                foreach ($request->subsystem as $row) {
                    $form[] = array(
                    'subsystem_id' => $row,
            ...
            return redirect()->back()->with([
                'message'    =>"Usercode is already used {$e->getMessage()}",
                'alert-type' => 'error',
            ]);
        }
    }
