...
class VwRFQ extends Model
{
    use HasFactory;
    protected $connection = 'sqlsrv_mmis';
    protected $table = 'CDG_MMIS.dbo.VwRFQ';
    protected $guarded = [];

    protected $appends = ['encrypted_key_id','remarks'];
    ...
    public function setKeyIdAttribute($value){
        $this->attributes['rfq_document_Reference_Number'] = Crypt::encrypt($value);
    }

    public function getEncryptedKeyIdAttribute()
    {
        return Crypt::encrypt($this->attributes['rfq_document_Reference_Number']);
    }

   // Accessor for the 'remarks' attribute
   public function getRemarksAttribute()
   {
       // Attempt to retrieve the related model
       $related = $this->remarks()->first();

       // Return the related attribute or a default value
       return $related ? $related->rfq_document_IntructionToBidders : null;
   }

   // Define the relationship
   public function remarks()
   {
       return $this->hasOne(QuotationMaster::class, 'rfq_document_Reference_Number', 'rfq_document_Reference_Number');
   }
}
