        ...
        <tbody>
          ...
          @foreach ($pdf_data['delivery']['items'] as $detail)
              <tr>
                ...
                <td class="item-td1" >{{ $detail['item']['item_name'] }}</td>
                <td class="item-td" >{{ (float)$detail['rr_Detail_Item_Qty_Received'] }}</td>
                <td class="item-td" >{{ $detail['unit']['name'] }}</td>
                <td class="item-td2 text-center" >{{ number_format($detail['rr_Detail_Item_ListCost'], 2) }}</td>
                <td class="item-td" >{{ number_format($detail['rr_Detail_Item_Vat_Amount'], 2) }}</td>
                <td class="item-td2 text-center" >{{ number_format($detail['rr_Detail_Item_TotalGrossAmount'], 2) }}</td>
                <td class="item-td2 text-center" >{{ number_format($detail['rr_Detail_Item_TotalDiscount_Amount'], 2) }}</td>
                <td class="item-td2 text-right" >{{ number_format($detail['rr_Detail_Item_TotalNetAmount'], 2) }} &nbsp;</td>
              </tr>
          @endforeach
          <tr>
            <td colspan="9" class="text-right" style="border:none !important;">TOTAL GROSS AMOUNT :</td>
            <td class="item-td item-border-bottom text-right">{{number_format($pdf_data['delivery']['rr_Document_TotalGrossAmount'], 2)}} &nbsp;</td>
          </tr>
          <tr>
            <td colspan="9" class="text-right" style="border:none !important;">TOTAL VAT :</td>
            <td class="item-td item-border-bottom text-right">{{number_format($pdf_data['delivery']['rr_Document_Vat_Inclusive_Rate'], 2)}} &nbsp;</td>
          </tr>
          <tr>
            <td colspan="9" class="text-right" style="border:none !important;">TOTAL DISCOUNT :</td>
            <td class="item-td item-border-bottom text-right" >{{number_format($pdf_data['delivery']['rr_Document_TotalDiscountAmount'], 2)}} &nbsp;</td>
          </tr>
       
          <tr>
            <td colspan="9" class="text-right" style="border:none !important;">TOTAL AMOUNT DUE : </td>
            <td class="item-td item-border-bottom text-right">{{number_format($pdf_data['delivery']['rr_Document_TotalNetAmount'], 2)}} &nbsp;</td>
          </tr>
        </tbody>
      </table>
