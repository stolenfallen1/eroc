...
@php
    ...
    foreach ($data as $item) {
        ...
        if ($item->statusdesc == 'POS -  Completed Order Sales') {
            ...
            if (isset($invoice_group[$invoice])) {
                // If the category_data exists, add the item to the category_data's array
                 $invoice_group[$invoice][] = $item;
            } else {
            // If the category doesn't exist, create a new array for the category and add the item
                $invoice_group[$invoice] = [$item];
            }
...
@php
    ...
    foreach ($invoice_group as $key => $invoice) {
        $totalvatable += (float) $invoice[0]->vatable;
        $totalvatexempt += (float) $invoice[0]->vatexempt;
        $totalvatamount += (float) $invoice[0]->vatamount;
        $totalvatsales += (float) $invoice[0]->vatexempt +  $invoice[0]->vatamount;
    }
@endphp
<body>
    ...
    <div>
        ...
        <div class="content-section">
            ...
            <div class="receipt-header text-center">
             ...
             <table class="summary">
                 {{-- <tr>
                     <td>Vat Include Sales</td>
                     <td  class="text-right">{{number_format($totalvatable,2)}}</td>
                 </tr> --}}
                 <tr>
